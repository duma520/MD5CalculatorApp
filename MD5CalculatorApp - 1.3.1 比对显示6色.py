            ...
            try:
                ...
                if self.compare_var.get() and self.compare_target.get():
                    if hash_value == self.compare_target.get().strip():
                        ...
                        self.multi_result_tree.tag_configure('match', background='#90EE90')  # 使用更明显的绿色
                ...
                for filepath in files:
                    for item in self.multi_result_tree.get_children():
                        ...
                        if values[0] == os.path.basename(filepath) and values[1] == os.path.dirname(filepath):
                            self.multi_result_tree.item(item, tags=(f'duplicate_{color_index}',))
                            self.multi_result_tree.tag_configure(f'duplicate_{color_index}', background=color)
                            break
